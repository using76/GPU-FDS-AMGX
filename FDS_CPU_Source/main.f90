...
! Initialize AmgX GPU solver library (must be after MPI_INIT)
#ifdef WITH_AMGX
CALL AMGX_INIT(IERR)
IF (IERR /= 0 .AND. MY_RANK == 0) THEN
   WRITE(LU_ERR,'(A)') ' WARNING: AmgX initialization failed, falling back to CPU solver'
ELSE
   ! Initialize GPU monitoring and print GPU info
   CALL AMGX_GPU_MONITOR_INIT(IERR)
   IF (MY_RANK == 0) THEN
      BLOCK
         CHARACTER(LEN=64) :: GPU_NAME
         REAL(EB) :: GPU_MEM_USED, GPU_MEM_TOTAL
         INTEGER :: GPU_IERR
         CALL AMGX_GET_GPU_NAME(GPU_NAME, GPU_IERR)
         CALL AMGX_GET_GPU_MEMORY(GPU_MEM_USED, GPU_MEM_TOTAL, GPU_IERR)
         WRITE(LU_ERR,'(A,A)')    ' GPU Device      : ', TRIM(GPU_NAME)
...
#ifdef WITH_AMGX
CALL AMGX_GPU_MONITOR_SHUTDOWN(IERR)
